...
class S2VTAttModel(nn.Module):
    def __init__(self, encoder, encoder_hand, encoder_voice, decoder):
        """
    ...
    def forward2(self, vid_feats, target_variable=None,
                mode='train', opt={}):
        """
        Args:
            vid_feats (Variable): video feats of shape [batch_size, seq_len, dim_vid]
    ...
    def forward3(self, vid_feats, hand_feats, hand_pro, target_variable=None,
        ...
        # encoder_outputs = torch.cat([encoder_outputs1, encoder_outputs2, encoder_outputs3], 2)
        # encoder_hidden = torch.cat([encoder_hidden1, encoder_hidden2, encoder_hidden3], 2)
        encoder_outputs = torch.cat([encoder_outputs1, encoder_outputs2], 2)
        encoder_hidden = torch.cat([encoder_hidden1, encoder_hidden2], 2)
        seq_prob, seq_preds = self.decoder(encoder_outputs, encoder_hidden, target_variable, mode, opt)
        return seq_prob, seq_preds

    def forward4(self, vid_feats, hand_feats, hand_pro, voice_feats, target_variable=None,
            mode='train', opt={}):
        encoder_outputs1, encoder_hidden1 = self.encoder(vid_feats)
        # todo 修改下面的vid_feats，变成声音和手语的特征,同时要传入参数
        encoder_outputs2, encoder_hidden2 = self.encoder_voice(voice_feats)
        ...
        # encoder_outputs = torch.cat([encoder_outputs1, encoder_outputs2, encoder_outputs3], 2)
        # encoder_hidden = torch.cat([encoder_hidden1, encoder_hidden2, encoder_hidden3], 2)
        encoder_outputs = torch.cat((torch.cat([encoder_outputs1, encoder_outputs3], 2), encoder_outputs2), 2)
        encoder_hidden = torch.cat((torch.cat([encoder_hidden1, encoder_hidden3], 2), encoder_hidden2), 2)
        seq_prob, seq_preds = self.decoder(encoder_outputs, encoder_hidden, target_variable, mode, opt)
        return seq_prob, seq_preds